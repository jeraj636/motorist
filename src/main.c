...
const char *fragment_shader_crte_p =
    ...
    "FragColor = vec4(1.0);\n"
    "}\n\0";
const char *vertex_shader_kvadrat_p =
    "#version 330 core\n"
    "layout (location = 0) in vec2 aPos;\n"
...
const char *fragment_shader_kvadrat_p =
    "#version 330 core\n"
    "out vec4 FragColor;\n"
    "in vec2 tvecpos;\n"
    ...
    "{\n"
    "//FragColor = vec4(1.0);\n"
    "FragColor = texture(textura, tvecpos);\n"
    "}\n\0";
...
int main()
{
    ...
    unsigned int kokos_slicica = nalozi_slicio("../slike/kokos.png");

    float tocke_kvadrat[6 * 4] = {
        -0.5, 0.5, 0, 1,
        0.5, 0.5, 1, 1,
        -0.5, -0.5, 0, 0,
        -0.5, -0.5, 0, 0,
    ...
    unsigned int program_za_kvadrate = naredi_program(vertex_shader_kvadrat_p, fragment_shader_kvadrat_p);
    ...
    while (!glfwWindowShouldClose(okno))
    {
        ...
        glUseProgram(program_za_kvadrate);
        ...
        glBindTexture(GL_TEXTURE_2D, kokos_slicica);

        glUniformMatrix3fv(glGetUniformLocation(program_za_kvadrate, "projekcija"), 1, GL_TRUE, &projekcija[0][0]);
        glUniformMatrix3fv(glGetUniformLocation(program_za_kvadrate, "kamera"), 1, GL_TRUE, &kamera[0][0]);
        glUniformMatrix3fv(glGetUniformLocation(program_za_kvadrate, "velikost"), 1, GL_TRUE, &velikost[0][0]);
        glUniformMatrix3fv(glGetUniformLocation(program_za_kvadrate, "pozicija"), 1, GL_TRUE, &pozicija[0][0]);
        glUniformMatrix3fv(glGetUniformLocation(program_za_kvadrate, "rotacija"), 1, GL_TRUE, &roatacija[0][0]);
        glUniform1i(glGetUniformLocation(program_za_kvadrate, "textura"), 0);
